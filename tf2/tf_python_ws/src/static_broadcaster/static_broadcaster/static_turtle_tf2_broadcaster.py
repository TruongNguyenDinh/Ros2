...
class StaticFramePublisher(Node):
    ...
    def make_transforms(self, transformation):
        ...
        t.transform.translation.x = float(transformation[2]) #Tạo độ x
        t.transform.translation.y = float(transformation[3]) #Tạo độ y
        t.transform.translation.z = float(transformation[4]) #Tạo độ z
        quat = quaternion_from_euler(
            float(transformation[5]), float(transformation[6]), float(transformation[7])) # Góc xoay roll - pitch - yaw
        # Lấy ra vector q
